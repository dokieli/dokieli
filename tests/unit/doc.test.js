    ...
    policyTypes.forEach((pT) => {
      if (pT == ns.odrl.Offer.value) {
        ...
        permissions.forEach((permissionIRI) => {
          ...
          info["odrl"][policyIRI]["permission"][permissionIRI]["action"] = info[
            "odrl"
      ...
      if (pT == ns.odrl.Agreement.value) {
        ...
        prohibition.forEach((prohibitionIRI) => {
          ...
          var prohibitionAssigner = prohibitionGraph.out(
            ns.odrl.assigner
          ).values;
          info["odrl"][policyIRI]["prohibition"][prohibitionIRI]["action"] =
            info["odrl"]["prohibitionAssigner"] = prohibitionAssigner;

          var prohibitionAssignee = prohibitionGraph.out(
            ns.odrl.assignee
          ).values;
          info["odrl"][policyIRI]["prohibition"][prohibitionIRI]["action"] =
            info["odrl"]["prohibitionAssignee"] = prohibitionAssignee;

          var prohibitionActions = prohibitionGraph.out(ns.odrl.action).values;
          info["odrl"][policyIRI]["prohibition"][prohibitionIRI]["action"] =
            info["odrl"]["prohibitionActions"] = prohibitionActions;
        });
      }
    });
  });
...
describe("getResourceInfoODRLPolicies with MockGrapoi", () => {
  it("processes Offer policies with permissions correctly", () => {
    ...
    const s = new MockGrapoi([
      {
        subject: { value: "root" },
        predicate: ns.odrl.hasPolicy,
        object: { value: policyIRI },
      },
      {
        subject: { value: policyIRI },
        predicate: ns.rdf.type,
        object: { value: ns.odrl.Offer.value },
      },
      {
        subject: { value: policyIRI },
        predicate: ns.odrl.permission,
        object: { value: permissionIRI },
      },

      {
        subject: { value: permissionIRI },
        predicate: ns.odrl.assigner,
        object: { value: "assigner1" },
      },
      {
        subject: { value: permissionIRI },
        predicate: ns.odrl.action,
        object: { value: "action1" },
      },
      {
        subject: { value: permissionIRI },
        predicate: ns.odrl.action,
        object: { value: "action2" },
      },
    ]);
    ...
    expect(info[policyIRI].permission[permissionIRI].action).toEqual([
      "action1",
      "action2",
    ]);
  ...
  it("processes Agreement policies with prohibitions correctly", () => {
    ...
    const s = new MockGrapoi([
      ...
      },
      {
        subject: { value: policyIRI },
        predicate: ns.rdf.type,
        object: { value: ns.odrl.Agreement.value },
      },
      {
        subject: { value: policyIRI },
        predicate: ns.odrl.prohibition,
        object: { value: prohibitionIRI },
      },

      {
        subject: { value: prohibitionIRI },
        predicate: ns.odrl.assigner,
        object: { value: "prohibitionAssigner1" },
      },
      {
        subject: { value: prohibitionIRI },
        predicate: ns.odrl.assignee,
        object: { value: "prohibitionAssignee1" },
      },
      {
        subject: { value: prohibitionIRI },
        predicate: ns.odrl.action,
        object: { value: "prohibitionAction1" },
      },
      {
        subject: { value: prohibitionIRI },
        predicate: ns.odrl.action,
        object: { value: "prohibitionAction2" },
      },
    ]);
  ...
  it("skips unknown policy types", () => {
    ...
    const s = new MockGrapoi([
      ...
      },
      {
        subject: { value: policyIRI },
        predicate: ns.rdf.type,
        object: { value: "http://example.org/UnknownPolicyType" },
      },
    ]);
    ...
    expect(info[policyIRI].rdftype).toEqual([
      "http://example.org/UnknownPolicyType",
    ]);
